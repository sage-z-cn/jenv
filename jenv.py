...
def cmd_help() -> None:
    print(f"{CYAN}jenv - Java environment manager{RESET}\n"
          ...
          "Commands:\n"
          "  list      列出所有已安装的 JDK\n"
          "  current   显示当前生效的 JAVA_HOME\n"
          "  use  <版本>  切换 JDK 版本（需管理员权限）\n"
          "  dirs      管理 JDK 扫描目录\n"
          "  help      显示此帮助信息\n"
          "\n"
          "Dirs subcommands:\n"
          "  dirs [list]          列出当前扫描目录\n"
          "  dirs add   <目录>     添加扫描目录\n"
          "  dirs remove <目录>    移除扫描目录\n"
          "  dirs reset           重置为默认目录\n"
          "\n"
          "Examples:\n"
          "  jenv list              # 查看可用的 JDK\n"
          "  jenv current           # 查看当前使用的 JDK\n"
          "  jenv use 17            # 切换至 JDK 17\n"
          "  jenv use 1.8           # 切换至 JDK 1.8\n"
          "  jenv dirs add D:\\jdks   # 添加自定义 JDK 目录\n"
          "\n"
